  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900">
      ...
      <div className="hidden lg:flex lg:flex-shrink-0">
        <div className={`flex flex-col transition-all duration-300 ${desktopSidebarCollapsed ? 'w-16' : 'w-64'}`}>
          <div className="flex flex-col h-0 flex-1 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700">
            <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
              <div className={`flex items-center flex-shrink-0 px-4 ${desktopSidebarCollapsed ? 'justify-center' : ''}`}>
                <Scale className="h-8 w-8 text-primary-600" />
              ...
              <nav className="mt-5 flex-1 px-2 space-y-1">
                {navigation.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={`h-6 w-6 ${desktopSidebarCollapsed ? '' : 'mr-3'}`} />
              ...
              <div className="px-2">
                <button
                  ...
                  className={`w-full flex items-center px-2 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-md ${
                    desktopSidebarCollapsed ? 'justify-center' : ''
                  }`}
                  title={desktopSidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}
                >
                  <Menu className={`h-6 w-6 transform transition-transform ${desktopSidebarCollapsed ? 'rotate-180' : ''} ${desktopSidebarCollapsed ? '' : 'mr-3'}`} />
      ...
      <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="flex items-center justify-between bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-3">
            <div className="flex items-center space-x-3">
              <button
                ...
                className="text-gray-500 hover:text-gray-600 dark:text-gray-400 dark:hover:text-gray-300 p-1 rounded-md"
              >
                <Menu className="h-6 w-6" />
              </button>
              <div className="flex items-center">
                <Scale className="h-6 w-6 text-primary-600" />
                <span className="ml-2 text-lg font-bold text-gray-900 dark:text-white">Legal Metrology</span>
              </div>
            </div>
        ...
        <main className="flex-1 overflow-y-auto focus:outline-none">
          <div className="py-6">
            <div className={`mx-auto px-4 sm:px-6 md:px-8 ${desktopSidebarCollapsed ? 'max-w-none' : 'max-w-7xl'}`}>
              <Outlet />
            </div>
          </div>
        </main>
